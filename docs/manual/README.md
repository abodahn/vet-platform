...
1. **There is no People / HR chapter in this manual.** `docs/manual/people.md` does not
   exist. The `hr` (`/hr/`), `attendance` (`/attendance/`) and `payroll` (`/payroll/`)
   modules have no screen reference, with one exception: the staff-account screens at
   `/hr/staff` are documented in **System § 5**, because that is where user logins are
   actually created. The Workflow Book's `people.md` covers the screen map and
   permission map for all three modules but stops before the workflows.
   Source: directory listing of `docs/manual/`; `docs/workflows/people.md:21-186`;
   `docs/manual/system.md:486`.

2. **Five registered modules have no chapter in either book** — Service catalogue
   (`/catalog/`), Doctor portal (`/doctor/`), Clinical decision support (`/cds/`), Data
   migration (`/migration/`) and the REST APIs (`/api/v1/`, `/api/public/`). `/cds/` is
   described only where the pharmacy screen calls it (Pharmacy A2); `/migration/` is
   named as out of scope in System § 1. WhatsApp (`/whatsapp/`) and Notifications
   (`/notifications/`) are now covered by **Comms** (`comms.md`).
   Source: `platform/app.py:237-290`; `platform/blueprints/catalog/routes.py`,
   `doctor/routes.py`, `cds/routes.py`, `migration/routes.py`, `api_v1/`, `public_api/`.
...
4. **Section numbering is not uniform across chapters.** Front Desk, Finance, Insights,
   Pet Shop and System number screens `§ N` from the chapter root; Clinical nests every
   screen under `§ 5` as `5.1`–`5.15`; Services uses a letter per module with numbered
   screens (`A1`, `B3`, `F5`); Pharmacy uses `A1`–`A6` / `B1`–`B8` / `C1`–`C7` / `D1`–`D3`.
   The index above uses each chapter's own scheme.
